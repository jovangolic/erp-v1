...
public interface TrackingInfoRepository extends JpaRepository<TrackingInfo, Long>, JpaSpecificationExecutor<TrackingInfo> {
    ...
    @Query("SELECT ti FROM TrackingInfo ti WHERE ti.shipment.originStorage.type = :type")
    List<TrackingInfo> findByShipment_OriginStorage_Type(@Param("type") StorageType type);
    @Query("SELECT ti FROM TrackingInfo ti WHERE ti.shipment.originStorage.status = :status")
    List<TrackingInfo> findByShipment_OriginStorage_Status(@Param("status") StorageStatus status);
}
