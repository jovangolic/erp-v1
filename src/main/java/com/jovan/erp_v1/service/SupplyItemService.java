...
public class SupplyItemService implements ISupplyItemService {
	...
	public SupplyItemResponse createSupplyItem(SupplyItemRequest request) {
		Procurement procurement = procurementRepository.findById(request.procurementId())
		        .orElseThrow(() -> new ProcurementNotFoundException("Procurement not found with id: " + request.procurementId()));
	...
	public SupplyItemResponse updateSupplyItem(Long id, SupplyItemRequest request) {
		SupplyItem supplyItem = supplyItemRepository.findById(id).orElseThrow(() -> new SupplyItemNotFoundException("SupplyItem not found wityh id: "+id));
		Procurement procurement = procurementRepository.findById(request.procurementId()).orElseThrow(() -> new ProcurementNotFoundException("Procurement not found wiht id: " + request.procurementId()));
		Vendor vendor = vendorRepository.findById(request.vendorId()).orElseThrow(() -> new SupplierNotFoundException("Vendor not found with id: " + request.vendorId()));
		supplyItem.setProcurement(procurement);
		supplyItem.setSupplier(vendor);
	...
	public SupplyItemResponse getOneById(Long supplyItemId) {
		SupplyItem response = supplyItemRepository.findById(supplyItemId).orElseThrow(() -> new SupplyItemNotFoundException("SupplyItem not found with id: "+supplyItemId));
	...
	public List<SupplyItemResponse> getBySupplierNameAndProcurementDateAndMaxCost(String supplierName,
		...
		return supplyItemRepository.findBySupplierNameAndProcurementDateAndMaxCost(supplierName, startDate, endDate, maxCost).stream()
				.map(supplyItemMapper::toResponse)
				.collect(Collectors.toList());
	}
}
