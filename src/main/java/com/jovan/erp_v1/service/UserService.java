...
public class UserService implements IUserService {
	...
	public void deleteUser(String email) {
		if(!userRepository.existsByEmail(email)) {
			throw new UserNotFoundException("Can't delete user with email: " + email);
		}
	...
	public User getUser(String email) {
	 ...
	 public User getUserByIdentifier(String identifier) {
		 if (identifier.contains("@")) {
			    // Email login
			 return userRepository.findByEmail(identifier).orElseThrow(() -> new UserNotFoundException("Email not found"));
			}  
	...
	public UserResponse createUserByAdmin(UserRequest request) {
	    // 1. Proveri da li već postoji korisnik sa istim email-om ili korisničkim imenom
	    if (userRepository.existsByEmail(request.email())) {
